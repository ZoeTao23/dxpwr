...
#'
#' Calculate the required sample size for estimating sensitivity at a fixed false positive rate (FPR) or specificity at a fixed false negative rate (FNR) of a diagnostic test.
#'
...
#' @examples
#' n1 <- sample_estimate_restricted_sesp(metric="sens", A=0.8, b=1, e=0.1, alpha=0.05, beta=NULL, L=0.1, R=1, dist="binorm")
#' print(n1)
#' n2 <- sample_estimate_restricted_sesp(metric="spec", A=0.8, b=1, e=0.1, alpha=0.05, beta=0.2, L=0.1, R=1, dist="binorm")
#' print(n2)
#' @export
sample_estimate_restricted_sesp <- function(metric, A, b=1, e, alpha=0.05, beta=NULL, L, R=1, dist="binorm") {
  ...
  se0 <- pnorm(a + b * qnorm(e))
  a_lwr <- qnorm(se0 - L) - b * qnorm(e)
  a_upr <- qnorm(se0 + L) - b * qnorm(e)
  #print(c(a_lwr, a_upr))

  ## calculate range of Z-value and delta
  z_lwr <- a_lwr + b * qnorm(e)
  z_upr <- a_upr + b * qnorm(e)
  z_delta <- (z_upr - z_lwr) / 2
  #print(c(z_lwr, z_upr, z_delta))
  ...
  html_report <- htmltools::tags$div(
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$p(
        ...
        "To evaluate the ",
        htmltools::tags$strong(style = paste0("color:", ifelse(A > 0.5, "#E74C3C", "#27AE60"), ";"), ifelse(metric=="sens", "sensitivity at fixed FPR", "specificity at fixed FNR")),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$div(
        ...
        if(metric=="sens") {
          htmltools::tags$p(
            "For estimating sensitivity with a given precision, the required sample size is calculated using the formula:"
          )
        }
        else{
          htmltools::tags$p(
            "For estimating specificity with a given precision, the required sample size is calculated using the formula:"
          )
        },
      ...
      htmltools::tags$div(
        ...
        htmltools::tags$h4(
          style = "color: #2980B9;",
          paste0("Step 3: Z-transformed ",ifelse(metric=="sens","sensitivity","specificity"))
        ),

        htmltools::tags$p(
          paste0("Define the z-transformed ", ifelse(metric=="sens","sensitivity at a FPR ","specificity at a FNR "), "of e as:")
        ),
        ...
        htmltools::tags$div(
          ...
          htmltools::tags$p(
            ...
            paste0("$$ z[Se_{",ifelse(metric=="sens","FPR","FNR"),"=e}] = a + b \\Phi^{-1}(e)$$")
          )
        ),

        htmltools::tags$p(
          paste0("So, the margin of error for z-transformed ",ifelse(metric=="sens","sensitivity","specificity")," is:")

        ),

        htmltools::tags$p(
          style = "text-align: center; margin: 10px 0; font-family: 'Cambria Math', serif; font-size: 1.1em;",
          paste0("$$ L(z[Se_{",ifelse(metric=="sens","FPR","FNR"),"=e}]) = \\dfrac{z[Se_{",ifelse(metric=="sens","FPR","FNR"), "=e} +L]-z[Se_{",ifelse(metric=="sens","FPR","FNR"),"=e}-L]}{2}$$")
        )
      ...
      htmltools::tags$div(
        ...
        if(dist=="binorm"){
          ...
          htmltools::tags$div(
            ...
            htmltools::tags$p(
              style = "font-family: 'Cambria Math', serif; font-size: 1.1em;",
              sprintf(
                paste0("$$ V(z[Se_{",ifelse(metric=="sens","FPR","FNR"),"=e}]) = 1 + \\dfrac{%.3f^2}{%.3f}+\\dfrac{%.3f^2}{2}+\\dfrac{%.3f^2 \\times \\Phi^{-1}(%.3f)\\times(1+%.3f)}{(2\\times %.3f)} = %.3f $$"),
                b, R, a, b, e, R, R, var_function
              )
            )
          )
          ...
          htmltools::tags$div(
            ...
            htmltools::tags$p(
              style = "font-family: 'Cambria Math', serif; font-size: 1.1em;",
              sprintf(
                paste0("$$ V(z[Se_{",ifelse(metric=="sens","FPR","FNR"),"=e}]) = 1 + \\dfrac{%.3f^2}{%.3f}+\\dfrac{%.3f^2}{2}+\\dfrac{%.3f^2 \\times \\Phi^{-1}(%.3f)\\times(1+%.3f)}{(2\\times %.3f)}+%.3f \\times %.3f \\times \\Phi^{-1}(%.3f) = %.3f $$"),
                b, R, a, b, e, R, R, a, b, e, var_function
              )
            )
          )
        ...
        htmltools::tags$p(
          "Next, compute the Margin of error term:"
        ),
        ...
        htmltools::tags$div(
          ...
          htmltools::tags$p(
            style = "font-family: 'Cambria Math', serif; font-size: 1.1em;",
            sprintf(
              paste0("$$ Se_{",ifelse(metric=="sens","FPR","FNR"),"=e} = \\Phi[%.3f + %.3f \\times \\Phi^{-1}(%.3f)] = %.3f  $$"),
              a, b, e, pnorm(a+b*qnorm(e))
            )
          )
        ),
        ...
        htmltools::tags$div(
          ...
          htmltools::tags$p(
            style = "font-family: 'Cambria Math', serif; font-size: 1.1em;",
            sprintf(
              paste0("$$ L([Se_{",ifelse(metric=="sens","FPR","FNR"),"=e}])= \\dfrac{\\Phi^{-1}(%.3f + %.3f) - \\Phi^{-1}(%.3f - %.3f)}{2} = %.3f $$"),
              se0, L, se0, L, z_delta
            )
          )
        ),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$h4(style = "margin-top: 0;", "Participants with vs. without condition"),
      ...
      htmltools::tags$p(
        ...
        paste0(" will be within ±", L*100, "% of the true value at fixed "),
        ifelse(metric=="sens", "FPR", "FNR"), paste0(" of ", e*100, "% "),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$p(
        ...
        htmltools::tags$br(),
        paste0("Sample size calculations for estimating",ifelse(metric=="sens","sensitivity at fixed FPR", "specificity at fixed FNR"))
      ),
      ...
      htmltools::tags$p(
        ...
        htmltools::tags$br(),
        "[3] Obuchowski NA, McClish DK. Sample Size Determination for Diagnostic Accuracy Studies Involving Binormal ROC Curve Indices. ",
  ...
  invisible(
    structure(
      list(
        ...
        parameters = list(A = A, a = a, b = b, e = e, alpha = alpha, beta = beta, L = L, R = R),
